...
function print_usage {
  echo
  echo "Usage: nexus [OPTIONS]"
  echo
  echo "This script is used to configure a Nexus instance."
  echo
  echo "Options:"
  echo
  echo -e "  --server-type\t\tType of server for integrations with other modules. Optional. Defaults to 'nexus'."
...
function generate_consul_config {
  ...
  local readonly consul_destination="${consul_config}/nexus.hcl"

  local readonly nexus_service=$(consul_kv "${consul_prefix}nexus/service_name")
  local readonly nexus_port=$(consul_kv "${consul_prefix}nexus/port")

  local readonly traefik_enabled=$(consul_kv_with_default "${consul_prefix}nexus/traefik/enabled" "no")

  local traefik_tags=""

  if [[ "$traefik_enabled" == "yes" ]]; then
    local readonly traefik_fqdns=$(consul_kv "${consul_prefix}nexus/traefik/fqdns")
    local readonly traefik_entrypoints=$(consul_kv "${consul_prefix}nexus/traefik/entrypoints")
...
service {
  ...
  check {
    name     = "Nexus Targets"
    http     = "http://127.0.0.1:${nexus_port}/index.html"
    method   = "GET"
    interval = "30s"
    timeout  = "2s"
  }
...
function main {
  ...
  local readonly data_device_name=$(consul_kv "${consul_prefix}nexus/data_device_name")
  local readonly db_dir=$(consul_kv "${consul_prefix}nexus/db_dir")
