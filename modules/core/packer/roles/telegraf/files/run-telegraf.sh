...
function main {
  ...
  if [[ "$enabled" != "yes" ]]; then
    ...
    if [[ "$skip_template" == "false" && -f "$conf_template" ]]; then
      local readonly consul_template_telegraf_conf=$(cat <<EOF
template {
  source = "${conf_template}"
  destination = "${conf_out}"
  perms = 0644
  backup = true
  command = "systemctl reload telegraf && systemctl restart telegraf"
}
EOF
)
      log_info "Add consul-template watch on \"$conf_template\" to generate \"$conf_out\"..."

      echo "$consul_template_telegraf_conf" > /opt/consul-template/config/telegraf.hcl
    ...
    # Added new config to consul, so reload (nomad and vault not supposed to be running yet)
